...
import { DocRepositoryMongo } from "../db/repositories/DocRepositoryMongo.js";
import { ChunkRepositoryMongo } from "../db/repositories/ChunkRepositoryMongo.js";
import { QdrantVectorRepository } from "../vector-store/QdrantVectorRepository.js";
import { CacheService } from "../services/adapters/cache-wrapper.service.js";
import { deletePdfFile } from "../services/core/file-storage.service.js";
import { countPointsInQdrant } from "../services/core/qdrant.service.js";
 ...
 * 
 * Uso: node src/infrastructure/scripts/delete-pdf.js <pdfId> [tenantId] [userId] [--dry-run] [--force]
 * 
 * Ejemplos:
 *   node src/infrastructure/scripts/delete-pdf.js 507f1f77bcf86cd799439011
 *   node src/infrastructure/scripts/delete-pdf.js 507f1f77bcf86cd799439011 --dry-run
 *   node src/infrastructure/scripts/delete-pdf.js 507f1f77bcf86cd799439011 --force
 *   node src/infrastructure/scripts/delete-pdf.js 507f1f77bcf86cd799439011 507f1f77bcf86cd799439012 507f1f77bcf86cd799439013 --dry-run
 */
...
const deletePdfCompletely = async () => {
  try {
    ...
    if (!pdfId) {
      ...
      console.log("\nUso: node src/infrastructure/scripts/delete-pdf.js <pdfId> [tenantId] [userId] [--dry-run] [--force]");
      console.log("\nEjemplos:");
      console.log("  node src/infrastructure/scripts/delete-pdf.js 507f1f77bcf86cd799439011");
      console.log("  node src/infrastructure/scripts/delete-pdf.js 507f1f77bcf86cd799439011 --dry-run");
      console.log("  node src/infrastructure/scripts/delete-pdf.js 507f1f77bcf86cd799439011 --force");
      process.exit(1);
    }
    ...
    const pdf = await DocModel.findById(pdfId).lean();
    ...
    if (!pdf) {
      ...
      console.log("   node src/infrastructure/scripts/list-pdfs.js");
    ...
    // Realizar hard delete usando el use case
    console.log("\n🗑️  Iniciando eliminación...");
    
    // Instanciar dependencias del use case
    const pdfRepository = new DocRepositoryMongo();
    const chunkRepository = new ChunkRepositoryMongo();
    const vectorRepository = new QdrantVectorRepository();
    const cacheService = new CacheService();
    
    // Crear y ejecutar use case
    const hardDeletePdfUseCase = new HardDeleteDocUseCase(
      pdfRepository,
      chunkRepository,
      vectorRepository,
      cacheService
    ...
    await hardDeletePdfUseCase.execute({
